...
class Pretraining(object):
    ...
    def warmup(self,
        ...
        ##############################################################
        if save is not None:
            if allow_resume and os.path.exists(f'{save}/warmup/chkpt.index'):
                self.model.load_weights(save + '/warmup/chkpt')
                with open(save + '/warmup/hp.json', 'r') as f:
                    data = json.load(f)
                frozen_layers, s_lr, e_lr, epochs, steps_per_epoch = data['frozen_layers'], data['s_lr'], data['e_lr'], data['epochs'], data['steps_per_epoch']

                print("Loading weights and resuming from end of previous warmup stage")
        ...
        callbacks = self.get_callbacks(lr_schedule=False)
