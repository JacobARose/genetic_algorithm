...
class Composable(Layers, Preprocess, Pretraining, HyperTune, Training, Dataset):
    ...
    def load_pretrained_weights(self, weights: str=None):
        if self.model is None:
            raise Exception('Must first instantiate model architecture prior to loading pretrained weights')
        if weights.endswith('chkpt'):
            if not os.path.isfile(weights+'.index'):
                raise Exception(f'Invalid or corrupt checkpoint file found at specified dir. Inspect files manually.')
        elif not os.path.exists(weights):
            raise Exception(f'No weights file found at specified path: {weights}')
        else:
            weights = os.path.join(weights,'chkpt')
                
        try:
            self._load_status = self._model.load_weights(weights)
        except Exception as e:
            raise e(f'Error encountered while attempting to load weights')
        print(f'Successfully loaded model weights from {weights}!')
