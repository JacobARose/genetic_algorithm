...
from tensorflow.keras.layers import ReLU, Dense, Conv2D, Conv2DTranspose, Softmax
from tensorflow.keras.layers import DepthwiseConv2D, SeparableConv2D, Dropout
from tensorflow.keras.layers import GlobalMaxPooling2D, GlobalAveragePooling2D, Activation, BatchNormalization
...
class Layers(object):
    ...
    # layers
    _conv = Conv2D
    GLOBAL_POOLING_LAYERS = {'avg':GlobalAveragePooling2D,
    ...
    def classifier(self, x, num_classes, **metaparameters):
...
#             metaparameters.pop('pooling')
        ...
        if pooling is not None:
            ...
            if isinstance(pooling, str):
                pooling = self.GLOBAL_POOLING_LAYERS[pooling]
            x = pooling()(x)
        ...
        x = self.Dense(x, num_classes, use_bias=True, **metaparameters)

        # Save the pre-activation probabilities layer
        self.probabilities = x
        outputs = Activation('softmax')(x)
#         outputs = Softmax(x.outputs)
    ...
    def freeze(self, model, frozen_layers=(0,-1)):
        """ Freeze all the layers in the model """
        start, end = frozen_layers
        frozen_i = []
        unfrozen_i = []
        for i, layer in enumerate(model.layers[start:end]):
            layer.trainable = False            
            assert model.layers[i].trainable is False
            frozen_i.append(i)

        if end not in [-1, len(model.layers)]:
            if end < 0:
                set_trainable = range(end,0,1)
            else:
                set_trainable = range(0,end,-1)
            for i in set_trainable:
                model.layers[i].trainable = True
                assert model.layers[i].trainable is True
                unfrozen_i.append(i)
